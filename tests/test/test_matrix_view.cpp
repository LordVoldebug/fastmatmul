...
TEST(MatrixViewBoundsDeath, OperatorAndGetter) {
  {
    ...
    auto V = A.View();
    EXPECT_DEATH({ (void)V(2,0); }, "");
    EXPECT_DEATH({ (void)V(0,3); }, "");
  }
  {
    linalg_lib::Matrix<int> A(2, 3);
    EXPECT_DEATH({ (void)A.SubMatrix(1,1,2,3); }, "");
    EXPECT_DEATH({ (void)A.Row(2); }, "");
    EXPECT_DEATH({ (void)A.Col(3); }, "");
  }
}
...
TEST(MatrixViewMultiplicationInplace, SquareSubmatrixAndNonSquareDeath) {
  ...
  }
  {
    ...
    auto V = A.SubMatrix(0, 0, 2, 3);
    EXPECT_DEATH({ V *= A.View(); }, "");
  }
}
