...
// транспонированием можно либо в одном и том же классе и ту и ту логику писать
...
// наоборот. И так проще, как минимум меньше методов прокидывать И в самом
// MatrixView про транспонирование нужно в принципе думать только в функции
// Transposed
...
class MaybeTransposedMatrixView {
 public:
  ...
  // удовольствие чтобы компилятор мог выбрать себя, а так читаемо а у этого
  // есть понятный физический смысл: такие касты разрешены они и в
  // std::reference_wrapper к примеру разрешены, просто сохраняем семантику
...
class MatrixView {
 ...
 private:
  ...
  // слайсы легкие объекты поэтому быть особо аккуратными с конструкторами тут
  // нет смыслы
 ...
 public:
  ...
  // В какой то момент меня смутили квалификаторы (и они стояли неверно)
